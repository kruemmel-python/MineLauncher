...
public class PlayerDataManager {
    ...
    private PlayerProfile loadProfile(UUID uuid) {
        ...
        if (config.contains(key)) {
            ...
            profile.setGuildId(section.getString("guildId", null));
            profile.setElo(section.getInt("elo", 1000));
            ConfigurationSection professions = section.getConfigurationSection("professions");
            if (professions != null) {
                for (String prof : professions.getKeys(false)) {
                    profile.professions().put(prof, professions.getInt(prof));
                }
            }
            ...
            ConfigurationSection bindings = section.getConfigurationSection("skillBindings");
            if (bindings != null) {
                for (String slotKey : bindings.getKeys(false)) {
                    try {
                        int slot = Integer.parseInt(slotKey);
                        profile.skillBindings().put(slot, bindings.getString(slotKey));
    ...
    public void saveProfile(PlayerProfile profile) {
        ...
        ConfigurationSection bindings = section.createSection("skillBindings");
        ...
        section.set("guildId", profile.guildId());
        section.set("elo", profile.elo());
        ConfigurationSection professions = section.createSection("professions");
        for (Map.Entry<String, Integer> entry : profile.professions().entrySet()) {
            professions.set(entry.getKey(), entry.getValue());
        }
        save();
    }
