...
public final class MyceliaChunkGenerator extends ChunkGenerator {
    ...
    public ChunkData generateChunkData(
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                int surfaceHeight = clampHeight(
                        seaLevel + (int) Math.round(heightNoise * 20.0),
                        minHeight + 1,
                        maxHeight - 1
                );
    ...
    public List<BlockPopulator> getDefaultPopulators(World world) {
        // Populator wird vom Server beim Chunk-Populate aufgerufen (versionstabil).
        ensureInitialized(world);
        return List.of(new MyceliaIslandsPopulator(world.getSeed()));
    }
